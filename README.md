...
Scriblism 是 Windows 11 的文字、程式碼與 Markdown 編輯器。可以同時打開多個檔案，用分頁切換；側欄顯示目前資料夾，方便在筆記和專案檔案之間移動。寫 Markdown 時，可在原始碼與預覽之間切換，並用大綱跳到標題。
...
- Markdown 原生預覽與原始碼切換；標題、粗斜體、刪除線、連結、引用、行內／圍欄程式碼及表格。預覽中的表格與正文一起捲動，儲存格文字會換行；點選區塊可切到 Markdown 原文編輯，Ctrl+E 可返回預覽。編輯選單可插入 Markdown 格式；大綱可跳到標題。語言與模式切換放在狀態列，不佔上方工具列。
...
- 設定頁可從「檢視 → 設定」或 Ctrl+, 開啟。一般／原始碼與 Markdown 預覽可各自設定多個字體；也可調整主題、自動換行，以及下次啟動時是否重新開啟上次的檔案。

## 邊界

**這版是可用的原生 Markdown 編輯器，不是完整 Typora 替代品。** 預覽與原始碼編輯是兩種模式；圖片仍保留語法且不內嵌，不執行 HTML、JavaScript、Mermaid，也不下載遠端內容。富文本的持久格式是 Markdown，沒有 RTF／DOCX 匯入匯出。
